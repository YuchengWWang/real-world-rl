...
class DualPiperEnv(gym.Env):
    ...
    #### 原step和reset函数反复销毁重建线程，开销大，这里改成线程复用形式运行两个env.step
    def step(self, action: np.ndarray) -> tuple:
        action_left = action[:len(action)//2]
        action_right = action[len(action)//2:]
        f_left  = self._pool.submit(self.env_left.step,  action_left)
        f_right = self._pool.submit(self.env_right.step, action_right)
    ...
    def reset(self, **kwargs):
        f_left  = self._pool.submit(self.env_left.reset,  **kwargs)
        f_right = self._pool.submit(self.env_right.reset, **kwargs)

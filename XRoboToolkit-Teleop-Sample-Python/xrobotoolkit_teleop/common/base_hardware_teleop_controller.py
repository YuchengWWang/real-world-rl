...
class HardwareTeleopController(BaseTeleopController, ABC):
    ...
    def _pub_pose_thread(self, stop_event: threading.Event):
        while not stop_event.is_set():
            start_time = time.time()
            self._update_xr_pub_pose()
            self._update_xr_pub_gripper()

            data = self.curr_action
            data_for_left= {"pose": data["left_arm"]["pose"],
                            "gripper": data["left_arm"]["gripper"],
                            "activate_button": data["left_arm"]["activate_button"]}
            data_for_right= {"pose": data["right_arm"]["pose"],
                            "gripper": data["right_arm"]["gripper"],
                            "activate_button": data["right_arm"]["activate_button"]}
            requests.post(url_left + "set_pico_action", json=data_for_left)
            requests.post(url_right + "set_pico_action", json=data_for_right)
            
            elapsed_time = time.time() - start_time
            sleep_time = (1.0 / self.control_rate_hz) - elapsed_time
            if sleep_time > 0:
                time.sleep(sleep_time)
        print("pub loop has stopped.")

    def _exe_thread(self, stop_event: threading.Event):
        while not stop_event.is_set():
            start_time = time.time()
            self._update_robot_state()
            self._update_curr_action()
            self._update_xr_exe_gripper()
            self._update_xr_exe_ik()
            if self.visualize_placo:
                self._update_placo_viz()
            elapsed_time = time.time() - start_time
            sleep_time = (1.0 / self.control_rate_hz) - elapsed_time
    ...
    def run(self):
        ...
        if self._is_xr_pub:
            core_threads = {
                ...
                "_pub_thread": self._pub_pose_thread
            }
        elif self._is_xr_exe:
            core_threads = {
                "_ik_thread": self._exe_thread,
                "_control_thread": self._control_thread,
            }
        else:
            core_threads = {

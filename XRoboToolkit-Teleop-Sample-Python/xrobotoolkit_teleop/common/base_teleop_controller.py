...
class BaseTeleopController(abc.ABC):
    def __init__(
        ...
        if _is_xr_pub:
            ...
            self.ref_ee_xyz = {name: None for name in self.manipulator_config.keys()}
            self.ref_ee_quat = {name: None for name in self.manipulator_config.keys()}
            self.ref_controller_xyz = {name: None for name in self.manipulator_config.keys()}
            self.ref_controller_quat = {name: None for name in self.manipulator_config.keys()}

            self.curr_action = {"right_arm": {"pose": [0., 0., 0., 0., .0, 0.],
                                              "gripper": [0.],
                                              "activate_button": [0.]},
            ...
            ps_left = requests.post(url_left+"get_p_ref_init").json()
            ps_right = requests.post(url_right+"get_p_ref_init").json()
            self.p_ref_init = ps_left["p_ref_init"] + ps_right["p_ref_init"] # 6Dright + 6Dlleft list
            self.curr_action["right_arm"]["pose"] = self.p_ref_init[:6]
            self.curr_action["left_arm"]["pose"] = self.p_ref_init[6:]
    ...
    def _process_xr_pose(self, xr_pose, src_name):
        ...
        if self.ref_controller_xyz[src_name] is None:
            ...
            if self._is_xr_pub:
            # ref last pose
                self.ref_controller_xyz[src_name] = controller_xyz * self.scale_factor
                self.ref_controller_quat[src_name] = controller_quat
    ...
    def _update_xr_pub_pose(self):
        for src_name, config in self.manipulator_config.items():
            xr_grip_val = self.xr_client.get_key_value_by_name(config["control_trigger"])
            self.curr_action[src_name]["activate_button"] = xr_grip_val
            self.active[src_name] = xr_grip_val > 0.9
    ...
    def _update_xr_pub_gripper(self):
        for gripper_name in self.manipulator_config.keys():
            ...
            if gripper_type == "parallel":
                ...
                for joint_name, open_pos, close_pos in zip(
                    ...
                    self.curr_action[gripper_name]["gripper"] = gripper_pos
            else:
                # TODO: add dexterous hand support
                raise ValueError(f"Unsupported gripper type: {gripper_type}")

    def _send_takeover(self):
        requests.post(url_left + "takeover")
        requests.post(url_right + "takeover")
    
    def _send_automatic(self):
        requests.post(url_left + "automatic")
        requests.post(url_right + "automatic")
    ...
    def _update_curr_action(self):
        ps_left = requests.post(url_left + "get_action").json()
        ps_right = requests.post(url_right + "get_action").json()
        self.curr_action["right_arm"]["pose"] = ps_right["pose"]
        self.curr_action["right_arm"]["gripper"] = ps_right["gripper"]
        self.curr_action["left_arm"]["pose"] = ps_left["pose"]
        self.curr_action["left_arm"]["gripper"] = ps_left["gripper"]
        # print(self.curr_action)

    def _update_xr_exe_gripper(self):
        for gripper_name in self.manipulator_config.keys():
            if "gripper_config" not in self.manipulator_config[gripper_name]:
                continue
            ...
            if gripper_type == "parallel":
                # trigger_value = self.xr_client.get_key_value_by_name(gripper_config["gripper_trigger"])
                # trigger_value = self.curr_action[gripper_name]["gripper"]
                joint_name = str(gripper_config["joint_names"][0])
                self.gripper_pos_target[gripper_name][joint_name] = self.curr_action[gripper_name]["gripper"]
            else:
                # TODO: add dexterous hand support
                raise ValueError(f"Unsupported gripper type: {gripper_type}")

    def _update_xr_exe_ik(self):
        self._update_robot_state()
        self.placo_robot.update_kinematics()

        for src_name, config in self.manipulator_config.items():
            # 这里持续执行不判断
            # xr_grip_val = self.curr_action[src_name]["activate_button"]
            # self.active[src_name] = xr_grip_val > 0.9 

            # if self.active[src_name]:
            if self.ref_ee_xyz[src_name] is None:
                print(f"{src_name} is activated.")
                self.ref_ee_xyz[src_name], self.ref_ee_quat[src_name] = self.p_ref_init

            action = np.array(self.curr_action[src_name]["pose"])
            delta_xyz = action[:3]
            delta_rot = action[3:]
                # print(delta_xyz)
            ...
            # self.ref_ee_xyz[src_name] = target_xyz
            # self.ref_ee_quat[src_name] = target_quat
